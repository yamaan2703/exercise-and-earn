import { Routes } from "@/routes/Routes";
import {
  Gender,
  OrderStatus,
  ProductDetailTab,
  StatusProduct,
  StatusUser,
  UserDetailTab,
} from "@/types/enums";
import { UserType, ProductType, OrderType, FaqType } from "@/types/interface";
import dynamic from "next/dynamic";
import {
  FaUsers,
  FaShieldAlt,
  FaFileContract,
  FaStore,
  FaBoxOpen,
  FaUser,
  FaFire,
  FaClipboardList,
} from "react-icons/fa";
import { FaBoxArchive } from "react-icons/fa6";
import { LuNotebookText } from "react-icons/lu";
import { MdDashboard } from "react-icons/md";
import { BiSolidHelpCircle } from "react-icons/bi";
import { TbTargetArrow } from "react-icons/tb";

export const sidebarMenu = [
  {
    label: "Dashboard",
    to: Routes.DASHBOARD,
    icon: <MdDashboard />,
  },
  {
    label: "Users",
    to: Routes.USERS,
    icon: <FaUsers />,
  },
  {
    label: "Products",
    to: Routes.PRODUCTS,
    icon: <FaStore />,
  },
  {
    label: "Goals",
    to: Routes.GOALS,
    icon: <TbTargetArrow />,
  },
  {
    label: "Orders",
    to: Routes.ORDERS,
    icon: <FaBoxOpen />,
  },
  {
    label: "Order History",
    to: Routes.ORDER_HISTORY,
    icon: <FaBoxOpen />,
  },
  {
    label: "Stock History",
    to: Routes.STOCK_HISTORY,
    icon: <FaClipboardList />,
  },
  {
    label: "Privacy Policy",
    to: Routes.PRIVACY_POLICY,
    icon: <FaShieldAlt />,
  },
  {
    label: "Terms & Conditions",
    to: Routes.TERMS_AND_CONDITIONS,
    icon: <FaFileContract />,
  },
  {
    label: "FAQs",
    to: Routes.FAQS,
    icon: <BiSolidHelpCircle />,
  },
];

export const UserTabs = [
  {
    key: UserDetailTab.PROFILE,
    label: "Profile",
    icon: <FaUser />,
  },
  {
    key: UserDetailTab.CALORIES,
    label: "Calories",
    icon: <FaFire />,
  },
  {
    key: UserDetailTab.PRODUCTS,
    label: "Claimed Products",
    icon: <FaStore />,
  },
];

export const ProductTabs = [
  {
    key: ProductDetailTab.INFO,
    label: "Info",
    icon: <LuNotebookText />,
  },
  {
    key: ProductDetailTab.STOCK,
    label: "Stock",
    icon: <FaBoxArchive />,
  },
];

export const ReactQuill = dynamic(() => import("react-quill-new"), {
  ssr: false,
});

export const modules = {
  toolbar: {
    container: [
      [{ header: [1, 2, 3, 4, 5, 6, false] }],
      [{ font: [] }],
      [{ size: [] }],
      ["bold", "italic", "underline", "strike", "blockquote", "code-block"],
      [{ color: [] }, { background: [] }],
      [
        { list: "ordered" },
        { list: "bullet" },
        { indent: "-1" },
        { indent: "+1" },
      ],
      [{ align: [] }],
      ["link", "image", "video"],
      ["clean"],
    ],
    handlers: {
      link: function (this: any, value: boolean) {
        if (value) {
          const href = prompt("Enter the URL");
          if (href) {
            const range = this.quill.getSelection();
            if (range && range.length > 0) {
              this.quill.format("link", href);
            } else {
              this.quill.insertText(range.index, href, "link", href);
            }
          }
        } else {
          this.quill.format("link", false);
        }
      },
    },
  },
};

export const formats = [
  "header",
  "font",
  "size",
  "bold",
  "italic",
  "underline",
  "strike",
  "blockquote",
  "code-block",
  "color",
  "background",
  "list",
  "indent",
  "align",
  "link",
  "image",
  "video",
];

export const initialFaqs: FaqType[] = [
  {
    id: 1,
    question: "What is “Burn Calories & Earn Gadgets”?",
    answer:
      "It’s a fitness reward program where you can track your burned calories and redeem them for exciting gadgets. You only cover the delivery cost—gadgets are free!",
  },
  {
    id: 2,
    question: "How do I participate?",
    answer:
      "Simply sign up, start tracking your calories, and redeem them for available gadgets in the rewards section.",
  },
  {
    id: 3,
    question: "What kind of gadgets can I earn?",
    answer:
      "You can earn fitness bands, wireless earbuds, smart bottles, and other exciting gadgets.",
  },
  {
    id: 4,
    question: "Do I really get gadgets for free?",
    answer:
      "Yes, all gadgets are free. You only need to pay the delivery charges.",
  },
  {
    id: 5,
    question: "How are calories tracked?",
    answer:
      "Calories are tracked automatically using your connected fitness device or by manual entry in the app.",
  },
  {
    id: 6,
    question: "Can I return or exchange a gadget?",
    answer: "No, gadgets once redeemed cannot be returned or exchanged.",
  },
];

export const dummyUsers: UserType[] = [
  {
    id: "1",
    name: "John Doe",
    gender: Gender.MALE,
    email: "john@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-300-1234567",
    createdAt: "2025-01-15",
    birthday: "1990-06-12",
    photoUrl: "https://randomuser.me/api/portraits/men/1.jpg",
    connectedDevice: "iOS",
    address: "123 Main Street Near City Park",
  },
  {
    id: "2",
    name: "Jane Smith",
    gender: Gender.FEMALE,
    email: "jane@example.com",
    status: StatusUser.INACTIVE,
    phone: "+92-301-7654321",
    createdAt: "2025-02-10",
    birthday: "1992-09-24",
    photoUrl: "https://randomuser.me/api/portraits/women/2.jpg",
    connectedDevice: "Android",
    address: "45 Oak Avenue Close To Market",
  },
  {
    id: "3",
    name: "Michael Lee",
    gender: Gender.MALE,
    email: "michael@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-302-1112233",
    createdAt: "2025-03-05",
    birthday: "1988-11-02",
    photoUrl: "https://randomuser.me/api/portraits/men/3.jpg",
    connectedDevice: "iOS",
    address: "78 Pine Street Opposite Grand Mall",
  },
  {
    id: "4",
    name: "Emily Davis",
    gender: Gender.FEMALE,
    email: "emily@example.com",
    status: StatusUser.INACTIVE,
    phone: "+92-303-4445566",
    createdAt: "2025-04-01",
    birthday: "1995-04-18",
    photoUrl: "https://randomuser.me/api/portraits/women/4.jpg",
    connectedDevice: "Android",
    address: "12 Rose Lane Behind Central Plaza",
  },
  {
    id: "5",
    name: "David Wilson",
    gender: Gender.MALE,
    email: "david@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-304-9998877",
    createdAt: "2025-04-20",
    birthday: "1991-12-09",
    photoUrl: "https://randomuser.me/api/portraits/men/5.jpg",
    connectedDevice: "iOS",
    address: "90 Maple Road Near River Bridge",
  },
  {
    id: "6",
    name: "Sophia Martinez",
    gender: Gender.FEMALE,
    email: "sophia@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-305-5556677",
    createdAt: "2025-05-02",
    birthday: "1993-08-30",
    photoUrl: "https://randomuser.me/api/portraits/women/6.jpg",
    connectedDevice: "Android",
    address: "34 Lake View Street Close To School",
  },
  {
    id: "7",
    name: "Daniel Brown",
    gender: Gender.MALE,
    email: "daniel@example.com",
    status: StatusUser.INACTIVE,
    phone: "+92-306-1237890",
    createdAt: "2025-05-15",
    birthday: "1990-01-21",
    photoUrl: "https://randomuser.me/api/portraits/men/7.jpg",
    connectedDevice: "iOS",
    address: "56 Garden Road Beside Old Museum",
  },
  {
    id: "8",
    name: "Olivia Taylor",
    gender: Gender.FEMALE,
    email: "olivia@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-307-9876543",
    createdAt: "2025-06-01",
    birthday: "1996-07-14",
    photoUrl: "https://randomuser.me/api/portraits/women/8.jpg",
    connectedDevice: "Android",
    address: "77 Hilltop Avenue Near City Hospital",
  },
  {
    id: "9",
    name: "James Anderson",
    gender: Gender.MALE,
    email: "james@example.com",
    status: StatusUser.INACTIVE,
    phone: "+92-308-1114455",
    createdAt: "2025-06-25",
    birthday: "1989-03-10",
    photoUrl: "https://randomuser.me/api/portraits/men/9.jpg",
    connectedDevice: "iOS",
    address: "101 River Side Street Close To Bridge",
  },
  {
    id: "10",
    name: "Ava Thomas",
    gender: Gender.FEMALE,
    email: "ava@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-309-2233445",
    createdAt: "2025-07-05",
    birthday: "1997-05-05",
    photoUrl: "https://randomuser.me/api/portraits/women/10.jpg",
    connectedDevice: "Android",
    address: "88 Elm Street Near Community Center",
  },
  {
    id: "11",
    name: "William Moore",
    gender: Gender.MALE,
    email: "william@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-310-7788990",
    createdAt: "2025-07-18",
    birthday: "1994-10-11",
    photoUrl: "https://randomuser.me/api/portraits/men/11.jpg",
    connectedDevice: "iOS",
    address: "65 Park Lane Opposite Railway Station",
  },
  {
    id: "12",
    name: "Isabella Jackson",
    gender: Gender.FEMALE,
    email: "isabella@example.com",
    status: StatusUser.INACTIVE,
    phone: "+92-311-3322114",
    createdAt: "2025-08-02",
    birthday: "1992-01-07",
    photoUrl: "https://randomuser.me/api/portraits/women/12.jpg",
    connectedDevice: "Android",
    address: "24 Cherry Blossom Street Behind Library",
  },
  {
    id: "13",
    name: "Ethan White",
    gender: Gender.MALE,
    email: "ethan@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-312-5566778",
    createdAt: "2025-08-15",
    birthday: "1991-09-19",
    photoUrl: "https://randomuser.me/api/portraits/men/13.jpg",
    connectedDevice: "iOS",
    address: "110 King Road Near Shopping Plaza",
  },
  {
    id: "14",
    name: "Mia Harris",
    gender: Gender.FEMALE,
    email: "mia@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-313-8899001",
    createdAt: "2025-09-01",
    birthday: "1998-02-26",
    photoUrl: "https://randomuser.me/api/portraits/women/14.jpg",
    connectedDevice: "Android",
    address: "39 Willow Avenue Close To Central Park",
  },
  {
    id: "15",
    name: "Alexander Martin",
    gender: Gender.MALE,
    email: "alexander@example.com",
    status: StatusUser.INACTIVE,
    phone: "+92-314-4455667",
    createdAt: "2025-09-10",
    birthday: "1987-12-20",
    photoUrl: "https://randomuser.me/api/portraits/men/15.jpg",
    connectedDevice: "iOS",
    address: "59 Broadway Street Behind Grand Hotel",
  },
  {
    id: "16",
    name: "Charlotte Thompson",
    gender: Gender.FEMALE,
    email: "charlotte@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-315-9988776",
    createdAt: "2025-09-22",
    birthday: "1996-06-01",
    photoUrl: "https://randomuser.me/api/portraits/women/16.jpg",
    connectedDevice: "Android",
    address: "18 Green Valley Road Near Main Market",
  },
  {
    id: "17",
    name: "Benjamin Garcia",
    gender: Gender.MALE,
    email: "benjamin@example.com",
    status: StatusUser.INACTIVE,
    phone: "+92-316-6677889",
    createdAt: "2025-10-02",
    birthday: "1990-08-23",
    photoUrl: "https://randomuser.me/api/portraits/men/17.jpg",
    connectedDevice: "iOS",
    address: "72 Sunset Boulevard Close To Stadium",
  },
  {
    id: "18",
    name: "Amelia Martinez",
    gender: Gender.FEMALE,
    email: "amelia@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-317-2233557",
    createdAt: "2025-10-14",
    birthday: "1994-03-04",
    photoUrl: "https://randomuser.me/api/portraits/women/18.jpg",
    connectedDevice: "Android",
    address: "200 Sunrise Road Near City Center Mall",
  },
  {
    id: "19",
    name: "Lucas Robinson",
    gender: Gender.MALE,
    email: "lucas@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-318-1117788",
    createdAt: "2025-11-01",
    birthday: "1993-05-15",
    photoUrl: "https://randomuser.me/api/portraits/men/19.jpg",
    connectedDevice: "iOS",
    address: "89 Silver Street Opposite Government College",
  },
  {
    id: "20",
    name: "Harper Clark",
    gender: Gender.FEMALE,
    email: "harper@example.com",
    status: StatusUser.INACTIVE,
    phone: "+92-319-3322445",
    createdAt: "2025-11-12",
    birthday: "1999-07-27",
    photoUrl: "https://randomuser.me/api/portraits/women/20.jpg",
    connectedDevice: "Android",
    address: "37 Golden Avenue Near River View Park",
  },
  {
    id: "21",
    name: "Henry Rodriguez",
    gender: Gender.MALE,
    email: "henry@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-320-5556677",
    createdAt: "2025-11-25",
    birthday: "1992-12-01",
    photoUrl: "https://randomuser.me/api/portraits/men/21.jpg",
    connectedDevice: "iOS",
    address: "141 Central Street Beside City Post Office",
  },
  {
    id: "22",
    name: "Evelyn Lewis",
    gender: Gender.FEMALE,
    email: "evelyn@example.com",
    status: StatusUser.INACTIVE,
    phone: "+92-321-7788991",
    createdAt: "2025-12-05",
    birthday: "1995-09-08",
    photoUrl: "https://randomuser.me/api/portraits/women/22.jpg",
    connectedDevice: "Android",
    address: "250 Forest Lane Opposite Shopping Plaza",
  },
  {
    id: "23",
    name: "Jack Walker",
    gender: Gender.MALE,
    email: "jack@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-322-3344556",
    createdAt: "2025-12-15",
    birthday: "1989-04-22",
    photoUrl: "https://randomuser.me/api/portraits/men/23.jpg",
    connectedDevice: "iOS",
    address: "300 Cedar Street Near Metro Bus Stop",
  },
  {
    id: "24",
    name: "Abigail Hall",
    gender: Gender.FEMALE,
    email: "abigail@example.com",
    status: StatusUser.ACTIVE,
    phone: "+92-323-6655443",
    createdAt: "2025-12-28",
    birthday: "1997-10-19",
    photoUrl: "https://randomuser.me/api/portraits/women/24.jpg",
    connectedDevice: "Android",
    address: "42 Magnolia Street Behind Police Station",
  },
  {
    id: "25",
    name: "Matthew Allen",
    gender: Gender.MALE,
    email: "matthew@example.com",
    status: StatusUser.INACTIVE,
    phone: "+92-324-1119998",
    createdAt: "2025-01-05",
    birthday: "1991-11-30",
    photoUrl: "https://randomuser.me/api/portraits/men/25.jpg",
    connectedDevice: "iOS",
    address: "75 Ashwood Lane Close To National Bank",
  },
];

export const dummyProducts: ProductType[] = [
  {
    id: "1",
    name: "Protein Powder",
    images: [
      "/images/watch.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Supplements",
    brand: "Optimum Nutrition",
    price: 45,
    requiredCalories: 100,
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-01",
    stock: 87,
    availableStock: 57,
    description:
      "High-quality protein powder designed to support muscle growth, recovery, and performance.",
  },
  {
    id: "2",
    name: "Yoga Mat",
    images: [
      "/images/bottle.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Fitness Gear",
    brand: "Liforme",
    price: 30,
    requiredCalories: 200,
    size: ["4mm", "6mm", "8mm"],
    color: ["Purple", "Blue", "Black"],
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-02",
    stock: 95,
    availableStock: 63,
    description: "Durable yoga mat with excellent grip and cushioning.",
  },
  {
    id: "3",
    name: "Dumbbells Set",
    images: [
      "/images/rope.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Equipment",
    brand: "Bowflex",
    price: 120,
    requiredCalories: 100,
    size: ["10kg", "15kg", "20kg"],
    color: ["Black", "Silver"],
    deliveryFee: 5,
    status: StatusProduct.INACTIVE,
    createdAt: "2025-12-03",
    stock: 72,
    availableStock: 49,
    description: "Adjustable dumbbells perfect for strength training at home.",
  },
  {
    id: "4",
    name: "Resistance Bands",
    images: [
      "/images/watch.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Accessories",
    brand: "TheraBand",
    price: 25,
    requiredCalories: 100,
    color: ["Red", "Green", "Blue", "Yellow"],
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-04",
    stock: 98,
    availableStock: 73,
    description:
      "Versatile resistance bands for stretching, rehab, and strength training.",
  },
  {
    id: "5",
    name: "Whey Protein",
    images: [
      "/images/bottle.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Supplements",
    brand: "MyProtein",
    price: 50,
    requiredCalories: 100,
    deliveryFee: 5,
    status: StatusProduct.INACTIVE,
    createdAt: "2025-12-05",
    stock: 61,
    availableStock: 39,
    description:
      "Premium whey protein that supports lean muscle growth and recovery.",
  },
  {
    id: "6",
    name: "Kettlebell",
    images: [
      "/images/rope.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Equipment",
    brand: "Rogue Fitness",
    price: 70,
    requiredCalories: 100,
    size: ["8kg", "12kg", "16kg", "20kg"],
    color: ["Black", "Gray"],
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-06",
    stock: 89,
    availableStock: 56,
    description:
      "Durable kettlebell for strength, endurance, and conditioning exercises.",
  },
  {
    id: "7",
    name: "Treadmill",
    images: [
      "/images/watch.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Equipment",
    brand: "NordicTrack",
    price: 900,
    requiredCalories: 200,
    size: ["Standard", "Large"],
    color: ["Black", "Gray"],
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-07",
    stock: 76,
    availableStock: 50,
    description:
      "Advanced treadmill with multiple speed levels and incline settings.",
  },
  {
    id: "8",
    name: "Creatine Monohydrate",
    images: [
      "/images/bottle.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Supplements",
    brand: "MuscleTech",
    price: 28,
    requiredCalories: 200,
    deliveryFee: 5,
    status: StatusProduct.INACTIVE,
    createdAt: "2025-12-08",
    stock: 92,
    availableStock: 58,
    description:
      "Pure creatine monohydrate to increase strength, performance, and muscle endurance.",
  },
  {
    id: "9",
    name: "Jump Rope",
    images: [
      "/images/rope.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Accessories",
    brand: "Crossrope",
    price: 15,
    requiredCalories: 100,
    color: ["Red", "Black", "Blue"],
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-09",
    stock: 81,
    availableStock: 53,
    description:
      "Lightweight jump rope designed for speed, cardio, and endurance training.",
  },
  {
    id: "10",
    name: "Foam Roller",
    images: [
      "/images/watch.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Accessories",
    brand: "TriggerPoint",
    price: 22,
    requiredCalories: 100,
    size: ["30cm", "45cm", "60cm"],
    color: ["Blue", "Black", "Gray"],
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-10",
    stock: 66,
    availableStock: 39,
    description: "Foam roller for deep tissue massage and muscle recovery.",
  },
  {
    id: "11",
    name: "BCAA Powder",
    images: [
      "/images/bottle.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Supplements",
    brand: "Scivation",
    price: 35,
    requiredCalories: 100,
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-11",
    stock: 84,
    availableStock: 54,
    description:
      "BCAA supplement to support muscle recovery and reduce fatigue.",
  },
  {
    id: "12",
    name: "Pull-Up Bar",
    images: [
      "/images/rope.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Equipment",
    brand: "Iron Gym",
    price: 45,
    requiredCalories: 200,
    deliveryFee: 5,
    status: StatusProduct.INACTIVE,
    createdAt: "2025-12-12",
    stock: 73,
    availableStock: 47,
    description:
      "Doorway pull-up bar suitable for upper body training at home.",
  },
  {
    id: "13",
    name: "Gym Gloves",
    images: [
      "/images/watch.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Accessories",
    brand: "Harbinger",
    price: 18,
    requiredCalories: 100,
    size: ["S", "M", "L", "XL"],
    color: ["Black", "Red"],
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-13",
    stock: 59,
    availableStock: 36,
    description:
      "Comfortable gloves with padding for weightlifting and training.",
  },
  {
    id: "14",
    name: "Mass Gainer",
    images: [
      "/images/bottle.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Supplements",
    brand: "Dymatize",
    price: 65,
    requiredCalories: 200,
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-14",
    stock: 90,
    availableStock: 58,
    description:
      "Calorie-dense mass gainer to support muscle growth and weight gain.",
  },
  {
    id: "15",
    name: "Stationary Bike",
    images: [
      "/images/watch.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Equipment",
    brand: "Peloton",
    price: 750,
    requiredCalories: 200,
    deliveryFee: 5,
    status: StatusProduct.INACTIVE,
    createdAt: "2025-12-15",
    stock: 62,
    availableStock: 39,
    description:
      "High-quality stationary bike with digital performance tracking.",
  },
  {
    id: "16",
    name: "Casein Protein",
    images: [
      "/images/bottle.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Supplements",
    brand: "ON",
    price: 55,
    requiredCalories: 100,
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-16",
    stock: 88,
    availableStock: 57,
    description: "Slow-digesting casein protein for overnight muscle recovery.",
  },
  {
    id: "17",
    name: "Weightlifting Belt",
    images: [
      "/images/rope.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Accessories",
    brand: "RDX",
    price: 35,
    requiredCalories: 100,
    size: ["M", "L", "XL"],
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-17",
    stock: 71,
    availableStock: 46,
    description:
      "Durable leather weightlifting belt for back support and heavy lifts.",
  },
  {
    id: "18",
    name: "Elliptical Machine",
    images: [
      "/images/watch.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Equipment",
    brand: "Sole",
    price: 1100,
    requiredCalories: 200,
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-18",
    stock: 64,
    availableStock: 41,
    description:
      "Full-body elliptical trainer for cardio and endurance workouts.",
  },
  {
    id: "19",
    name: "Fish Oil",
    images: [
      "/images/bottle.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Supplements",
    brand: "Nordic Naturals",
    price: 20,
    requiredCalories: 100,
    deliveryFee: 5,
    status: StatusProduct.INACTIVE,
    createdAt: "2025-12-19",
    stock: 77,
    availableStock: 50,
    description:
      "Omega-3 fish oil capsules to support heart, brain, and joint health.",
  },
  {
    id: "20",
    name: "Medicine Ball",
    images: [
      "/images/rope.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Accessories",
    brand: "CAP Barbell",
    price: 40,
    requiredCalories: 100,
    size: ["4kg", "6kg", "8kg"],
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-20",
    stock: 68,
    availableStock: 44,
    description:
      "Durable medicine ball designed for functional fitness training.",
  },
  {
    id: "21",
    name: "Rowing Machine",
    images: [
      "/images/watch.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Equipment",
    brand: "Concept2",
    price: 950,
    requiredCalories: 200,
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-21",
    stock: 85,
    availableStock: 55,
    description: "High-performance rowing machine with air resistance system.",
  },
  {
    id: "22",
    name: "Multivitamins",
    images: [
      "/images/bottle.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Supplements",
    brand: "Centrum",
    price: 18,
    requiredCalories: 100,
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-22",
    stock: 99,
    availableStock: 74,
    description:
      "Daily multivitamin tablets to support overall health and wellness.",
  },
  {
    id: "23",
    name: "Ankle Weights",
    images: [
      "/images/rope.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Accessories",
    brand: "Valeo",
    price: 27,
    requiredCalories: 100,
    size: ["2kg", "4kg", "6kg"],
    deliveryFee: 5,
    status: StatusProduct.INACTIVE,
    createdAt: "2025-12-23",
    stock: 60,
    availableStock: 37,
    description: "Adjustable ankle weights to add resistance during workouts.",
  },
  {
    id: "24",
    name: "Pre-Workout",
    images: [
      "/images/bottle.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Supplements",
    brand: "C4",
    price: 42,
    requiredCalories: 100,
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-24",
    stock: 83,
    availableStock: 53,
    description:
      "Explosive pre-workout formula to boost energy and performance.",
  },
  {
    id: "25",
    name: "Punching Bag",
    images: [
      "/images/watch.png",
      "/images/watch.png",
      "/images/bottle.png",
      "/images/rope.png",
    ],
    category: "Equipment",
    brand: "Everlast",
    price: 150,
    requiredCalories: 200,
    size: ["40kg", "60kg"],
    deliveryFee: 5,
    status: StatusProduct.ACTIVE,
    createdAt: "2025-12-25",
    stock: 74,
    availableStock: 47,
    description:
      "Heavy-duty punching bag for boxing and martial arts training.",
  },
];

export const initialOrders: OrderType[] = [
  {
    product: dummyProducts[0],
    user: dummyUsers[0],
    orderNumber: 1,
    orderStatus: OrderStatus.DELIVERED,
    date: "2025-09-10",
    paymentType: "Online",
  },
  {
    product: dummyProducts[1],
    user: dummyUsers[1],
    orderNumber: 2,
    orderStatus: OrderStatus.PENDING,
    date: "2025-09-12",
    paymentType: "Online",
  },
  {
    product: dummyProducts[2],
    user: dummyUsers[2],
    orderNumber: 3,
    orderStatus: OrderStatus.SHIPPED,
    date: "2025-07-16",
    paymentType: "Cash",
  },
  {
    product: dummyProducts[3],
    user: dummyUsers[3],
    orderNumber: 4,
    orderStatus: OrderStatus.DELIVERED,
    date: "2025-08-22",
    paymentType: "Cash",
  },
  {
    product: dummyProducts[4],
    user: dummyUsers[4],
    orderNumber: 5,
    orderStatus: OrderStatus.SHIPPED,
    date: "2025-09-01",
    paymentType: "Online",
  },
  {
    product: dummyProducts[5],
    user: dummyUsers[5],
    orderNumber: 6,
    orderStatus: OrderStatus.PENDING,
    date: "2025-03-12",
    paymentType: "Cash",
  },
  {
    product: dummyProducts[6],
    user: dummyUsers[6],
    orderNumber: 7,
    orderStatus: OrderStatus.SHIPPED,
    date: "2025-11-22",
    paymentType: "Cash",
  },
  {
    product: dummyProducts[7],
    user: dummyUsers[7],
    orderNumber: 8,
    orderStatus: OrderStatus.PENDING,
    date: "2025-10-16",
    paymentType: "Cash",
  },
  {
    product: dummyProducts[8],
    user: dummyUsers[8],
    orderNumber: 9,
    orderStatus: OrderStatus.DELIVERED,
    date: "2025-05-28",
    paymentType: "Online",
  },
  {
    product: dummyProducts[9],
    user: dummyUsers[9],
    orderNumber: 10,
    orderStatus: OrderStatus.PENDING,
    date: "2025-07-10",
    paymentType: "Online",
  },
];
